...
import { bhPostaCall, clubProfile, createEmptyProject, matchEligibility, projectStatus, validateProject, type Project } from '@/lib/proposal-model'
...
export default function Home() {
  ...
  return <main className="shell">
    ...
    <div className="workspace">
      ...
      <section className="eligibility-gate panel"><div className="panel-heading"><div><span className="step-number">02</span><h2>Analiza podobnosti poziva</h2></div><span className={`readiness ${eligibility.status === 'eligible' ? 'ready' : 'draft'}`}><span /> {eligibility.status === 'eligible' ? 'ELIGIBLE' : 'PROVJERA'}</span></div><p className="gate-lead">Analiza poziva se završava prije unosa projekta. Matcher provjerava formalne uslove, oblast, teritoriju i rok.</p><div className="gate-grid"><div><small>PREDLOŽENA KOMPONENTA</small><strong>{eligibility.recommendedProgram ?? 'Nema podudaranja'}</strong></div><div><small>OSNOV ZAKLJUČKA</small><span>{eligibility.reasons[0]}</span></div></div>{eligibility.risks.length > 0 && <div className="notice"><CircleAlert size={17} /><span><b>Rizici prije nastavka:</b> {eligibility.risks.join(' ')}</span></div>}<div className="gate-actions"><span>{eligibility.callPoints.length} tačaka poziva provjereno</span><button className="generate-btn" type="button" onClick={() => setGateConfirmed(true)} disabled={eligibility.status !== 'eligible' || gateConfirmed}>{gateConfirmed ? 'Komponenta potvrđena' : `Nastavi i generiši za ${eligibility.recommendedProgram ?? 'odabranu oblast'}`}<ChevronRight size={18} /></button></div></section>
      <div className="layout-grid">
        <section className="panel input-panel"><div className="panel-heading"><div><span className="step-number">01</span><h2>Učitajte projektni poziv</h2></div><span className="quiet-label">2 ekrana</span></div>
